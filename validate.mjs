...
const designOnlyResponse = await workerDefault.fetch(new Request("https://example.test/api/identify",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({obverse:null,reverse:"data:image/jpeg;base64,AA=="})}),{
  AI:{run:async(_model,input)=>{
    ...
    if(!Array.isArray(parts)||parts.find(part=>part.type==="image_url")?.image_url?.url!=="data:image/jpeg;base64,AA==")throw new Error("expected multimodal message input");
...
const checks = [
  [app.includes('APP_VERSION = "0.13.10"') && identify.includes('IDENTIFY_VERSION = "0.13.10"') && html.includes("Pocket Mint v0.13.10"), "v0.13.10 Home wordmark version"],
  ...
  [sixRoosVisual.status === 200 && sixRoosVisual.payload.uncertain && sixRoosVisual.payload.matches.length===0, "standard kangaroo observation cannot become a photo-identification result"],
  [misleadingRoosMatches.length===0 && misleadingRoosAssessment.uncertain, "generic five-kangaroo reading cannot create a catalogue suggestion"],
  ...
  [sixRoosMatches.length===0 && assessMatches(sixRoosMatches,visualCase({year:"2026",portrait:"King Charles III",design:"Mob of Six Roos"}),catalogue.coins).uncertain, "Mob of Six Roos remains catalogue-only"],
  ...
  [countedSixRoosMatches.length === 0, "kangaroo count cannot select a standard catalogue entry"],
  ...
  [sixRoosClues.length === 0, "standard kangaroo clues remain catalogue-only"],
  [genericRoosClues.length === 0, "generic kangaroo wording cannot create any match"],
  ...
  [identify.includes("prepareIdentifyPhoto") && identify.includes("decodeIdentifyPhoto") && html.includes('id="identifyReverseCamera" type="file" accept="image/*" capture="environment"') && identify.includes("readIdentifyInput"), "iPhone-safe photo preparation with separate native-camera input"],
  [worker.includes('type:"image_url"') && worker.includes("image_url:{url:image}"), "Llama 4 receives the photo inside its multimodal message"],
  [html.includes('id="coinCameraGuide"') && html.includes('id="coinCameraZoom"') && html.includes('id="reverseCapture"><button class="capturePreview" type="button" data-camera-side="reverse"') && html.includes('id="identifyGuidedCamera" class="textButton guidedCameraButton" type="button" data-native-camera-side="reverse"') && identify.includes("openNativeCoinCamera") && !identify.includes("focusCoinCamera") && !html.includes('id="coinCameraFocus"'), "fixed-circle guided camera is primary with native phone camera fallback"],
  ...
  [sw.includes("pocket-mint-v0.13.10") && sw.includes("./pm-visual.css") && sw.includes("./pm-wordmark.webp") && sw.includes("!/^https?") && sw.includes("./icons/character-512.png"), "matching service-worker cache, visual system, icon assets and remote image exclusions"],
  ...
  [catalogue.meta.catalogue_version === "0.8.1" && catalogue.coins.length === 223 && catalogue.coins.some(coin => coin.id === "AU1-2025-ROOS-KC3"), "expanded catalogue version and record total"],
  ...
  [catalogue.coins.filter(coin => coin.test_scope === "circulation_sample").length === 48 && catalogue.coins.filter(coin => coin.test_scope === "circulation_core").length === 10 && catalogue.coins.filter(coin => coin.test_scope === "circulation_partner").length === 165, "historical, recent and partner-program circulating totals"],
  ...
  [catalogue.coins.filter(coin => coin.series_id === "matildas").length === 4 && catalogue.coins.filter(coin => coin.series_id === "afl2023").length === 22 && catalogue.coins.filter(coin => coin.series_id === "afl2024").length === 2 && catalogue.coins.filter(coin => coin.series_id === "aussie_big_things").length === 11 && catalogue.coins.filter(coin => coin.series_id === "aussie_big_things_2").length === 12, "Matildas, AFL and both Aussie Big Things sets are present"],
  [catalogue.coins.some(coin => coin.id === "AU1-2024-AFL-PREMIERSHIP" && coin.mintage === 1250000) && catalogue.coins.some(coin => coin.id === "AU1-2024-AFLW-PREMIERSHIP" && coin.mintage === 1250000) && catalogue.coins.some(coin => coin.id === "AU1-2025-AUSSIE_BIG_THINGS_2-TILL-1") && catalogue.coins.some(coin => coin.id === "AU1-2025-AUSSIE_BIG_THINGS_2-TILL-2"), "missing 2024 AFL/AFLW and 2025 Big Things 2 till coins are present"],
