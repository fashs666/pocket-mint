...
- Uses the existing combined Grim and Noxel icon as a small Home cameo; a separate character asset can replace it later.
...
- makes the phone’s native camera the default when tapping either coin side
- restores reliable native autofocus, tap focus, lens selection and zoom
...
- only shows a green focus confirmation after the camera accepts the constraint
