...
function detailHtml(coin, record) {
  ...
  return `<section class="referencePanel pm-detail-art" aria-label="Catalogue coin artwork"><div class="pm-detail-artwork"><div class="eyebrow">${esc(referenceLabel(coin).toUpperCase())}</div>${coinImageHtml(coin, {preferPersonal: false, className: "detailArtwork"})}</div><p>${esc(imageNote)}</p></section>
    <header class="pm-detail-heading pm-cream-card"><div class="eyebrow">COIN DETAILS · ${esc(coin.id)}</div><h2>${multiYear ? esc(coin.title) : `${coin.year} ${esc(coin.title)}`}</h2><p class="muted">${multiYear ? `${variants.length} catalogue issues · ` : ""}${human(coin.coin_class)} · ${human(coin.issue_type)}</p>${yearControl}</header>
    <section class="pm-detail-information pm-cream-card"><h3>Coin information</h3><div class="detailGrid"><div><span>Year</span><b>${esc(variantIssueLabel(coin,variants))}</b></div><div><span>Denomination</span><b>$1</b></div><div><span>Mintage</span><b>${coin.mintage ? Number(coin.mintage).toLocaleString() : esc(coin.mintage_status || "—")}</b></div><div><span>Composition</span><b>${esc(coin.composition || "—")}</b></div><div><span>Size</span><b>${coin.mass_grams ?? "—"} g · ${coin.diameter_mm ?? "—"} mm</b></div><div><span>Effigy</span><b>${esc(coin.obverse_effigy || "—")}</b></div></div></section>
